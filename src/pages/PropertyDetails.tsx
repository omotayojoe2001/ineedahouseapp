    ...
    const fetchProperty = async () => {
      ...
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from('profiles')
          .select('first_name, last_name, full_name, phone, whatsapp_number, created_at, avatar_url')
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => {
              ...
              if (user.id === property.user_id) {
                toast({ title: "Cannot message yourself", description: "This is your own listing", variant: "destructive" });
                return;
              }
              navigate(`/messages/${property.user_id}`);
            }}>
      ...
      <div className="p-4 border-t border-border">
        ...
        <p className="text-sm text-muted-foreground mb-3">{property.address || property.location}</p>
        <div className="bg-muted rounded-lg h-64 overflow-hidden">
          <iframe
            ...
            frameBorder="0"
            style={{ border: 0 }}
            src={`https://www.google.com/maps/embed/v1/place?key=${import.meta.env.VITE_GOOGLE_PLACES_API_KEY}&q=${encodeURIComponent(property.address || property.location || 'Lagos, Nigeria')}`}
            allowFullScreen
          />
        </div>
      </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-border p-4 shadow-lg">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="lg" className="bg-primary hover:bg-primary-hover text-white" onClick={() => {
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white" onClick={() => {
              if (property.profiles?.whatsapp_number) {
                window.open(`https://wa.me/${property.profiles.whatsapp_number.replace(/[^0-9]/g, '')}`, '_blank');
              } else {
                toast({
                  title: "WhatsApp Unavailable",
                  description: "Owner has not added WhatsApp number",
                });
              }
            }}>
